...
for k in range(len(dataset)):
    ...
    filename = TRAINFOLDER + str(dataset[k])
    jsonname = 'train/annos/' + str(dataset[k][0:6]) + '.json'
    img = cv2.imread(filename)
    cv2.imwrite(YOLOTRAINFOLDER + str(dataset[k]), img)
    ...
    with open(jsonname, 'r') as f:
        ...
        for i in temp:
            if i == 'source' or i=='pair_id':
                ...
                with open(YOLOTRAINFOLDER + str(dataset[k][0:6]) + '.txt', 'w+') as f:
